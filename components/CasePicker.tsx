...
export function CasePicker({
  ...
  /** resolved on the server; a client component cannot hold a translator */
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="mt-1">
                  <Tag tone={c.scenario.tone}>{tr(c.scenario.label)}</Tag>
                </div>
              </div>
              <div className="shrink-0 text-right">
                ...
                <p className="text-[11px] font-bold uppercase tracking-wider text-ink-400">{tr('days')}</p>
              </div>
            </div>

            <p className="mt-3.5 flex-1 text-[14px] leading-relaxed text-ink-700">{tr(c.headline ?? '')}</p>

            <div className="mt-4 flex items-center justify-between border-t border-ink-100 pt-3.5 text-[12.5px] text-ink-400">
              <span className="tabular">{tr('UAN')} {c.uan}</span>
              <span className="inline-flex items-center gap-1 font-semibold text-teal-700 opacity-0 transition group-hover:opacity-100">
                {tr('Sign in')} <Icon name="route" size={13} aria-hidden />
              </span>
            </div>
