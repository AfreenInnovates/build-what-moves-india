...
export default async function EmploymentPage() {
  ...
  return (
    <main className="px-5 pb-28 pt-7 lg:pl-9 lg:pr-6">
      <PageHead
        icon="employment"
        title={t("My Employment")}
        lead={t(`Every job that paid into your PF, newest first. Problems are almost always created here, at one particular company, years before anyone tells you. This page says which job, and how long it has been sitting there.`)}
      />
      ...
      <div className="mt-6 max-w-[860px]">
        <p className="mb-4 rounded-md border-l-4 border-teal-700 bg-teal-50 px-4 py-3 text-[14.5px] leading-relaxed text-teal-900">
          {fill(
            t('Your record spans {jobs} jobs and {span} of work, from {first} to {last}.'),
            {
              jobs: String(story.employerCount),
              span: monthsInWords(story.totalService, t),
              first: story.fromEmployer,
              last: story.toEmployer,
            },
          )}
        </p>
        ...
        {earliest && (
          <div className="mb-6 rounded-lg border-l-4 border-stop bg-stop-soft px-4 py-3.5">
            <p className="flex items-center gap-2 text-[13px] font-bold uppercase tracking-[0.07em] text-stop">
              ...
              {t('Where it first went wrong')}
            </p>
            <p className="mt-2 text-[15px] leading-relaxed text-ink-800">
              {fill(
                earliest.problemAgeMonths !== null
                  ? t(
                      'The earliest problem on your record starts at {employer}, your {nth} job out of {total} - and it has been sitting there for {span}. Nothing would have told you until the day your claim was refused.',
                    )
                  : t(
                      'The earliest problem on your record starts at {employer}, your {nth} job out of {total}. Nothing would have told you until the day your claim was refused.',
                    ),
                {
                  employer: earliest.name,
                  nth: ordinal(earliest.position),
                  total: String(story.employerCount),
                  span:
                    earliest.problemAgeMonths !== null
                      ? monthsInWords(earliest.problemAgeMonths, t)
                      : '',
                },
              )}
            </p>
            {flaggedRows.length > 1 && (
              <p className="mt-1.5 text-[14px] leading-relaxed text-ink-700">
                {fill(t('{bad} of your {total} jobs have something that needs fixing. They are marked below.'), {
                  bad: String(flaggedRows.length),
                  total: String(story.employerCount),
                })}
              </p>
            )}
                ...
                >
                  ...
                  {flagged && e.problemAgeMonths !== null && (
                    <p className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-stop-soft px-2.5 py-1 text-[13px] font-semibold text-stop">
                      ...
                      Wrong for {monthsInWords(e.problemAgeMonths, t)} without anyone telling you
                    </p>
                  )}
                  ...
                  {e.gapBefore > 0 && (
                    <p className="mt-2 flex gap-2 rounded-sm bg-wait-soft px-3 py-2 text-[13px] text-ink-800">
                      ...
                      <span>
                        For <span className="font-semibold">{monthsInWords(e.gapBefore, t)}</span> {t('before you started here, nothing was paid into your PF. Those months do not count towards your pension.')}
                      </span>
                    </p>
                  )}
                  {e.exitMissing && (
                    <p className="mt-2 flex gap-2 rounded-sm bg-stop-soft px-3 py-2 text-[13px] text-ink-800">
                      ...
                      <span>
                        {t('Nobody recorded the day you left this job. Until someone does, EPFO counts you as still working here and will not release your money.')}
                      </span>
                    </p>
                  )}
                  {e.onSecondUan && (
                    <p className="mt-2 flex gap-2 rounded-sm bg-stop-soft px-3 py-2 text-[13px] text-ink-800">
                      <Icon name="alerts" size={15} className="mt-0.5 shrink-0 text-stop" aria-hidden />
                      <span>
                        {t('This job was put under a different PF number')} ({e.uan}){' '}
                        {t('instead of your main one')} ({primaryUan}).{' '}
                        {t('Your savings and your years are split across two accounts, and they have to be joined back together.')}
                      </span>
                    </p>
                  )}
                </div>
