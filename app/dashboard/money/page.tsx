    ...
    <main className="px-5 pb-28 pt-7 lg:pl-9 lg:pr-6">
      ...
      <div className="mt-6 max-w-[860px] space-y-6">
        <div className="grid gap-3 sm:grid-cols-3">
          <StatTile label="EPF balance" value={inr(m.balance)} tone="go" sub="Your withdrawable savings" />
          <StatTile label="Across" value={monthsToYM(p.serviceMonths)} sub="of recorded service" />
          <StatTile
            label="Rough monthly build-up"
            value={inr(m.monthlyEstimate)}
            sub="Estimate: balance ÷ months"
          />
        </div>

        <MoneyTimeline data={timeline} />

        <section className="rounded-md border border-ink-100 bg-white p-5">
          <h2 className="text-[16px] font-bold text-ink-900">Two pots, one deduction</h2>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <div className="rounded-sm bg-go-soft px-4 py-3.5">
              <p className="text-[13px] font-bold uppercase tracking-[0.06em] text-go">EPF · savings</p>
              <p className="mt-1 text-[14px] leading-relaxed text-ink-800">
                A lump sum you can withdraw. This is the <span className="font-bold">{inr(m.balance)}</span>{' '}
                above. It earns interest each year.
              </p>
            </div>
            <div className="rounded-sm bg-teal-50 px-4 py-3.5">
              <p className="text-[13px] font-bold uppercase tracking-[0.06em] text-teal-700">EPS · pension</p>
              <p className="mt-1 text-[14px] leading-relaxed text-ink-800">
        ...
        <section className="rounded-md border border-ink-100 bg-white p-5">
          <h2 className="text-[16px] font-bold text-ink-900">Where it came from</h2>
          ...
          <p className="mt-3 text-[12.5px] text-ink-500">
            Contributions from every employer belong in one account. If any sits under a second UAN,
            it is not counted here until merged.
          </p>
        </section>
      </div>
    </main>
